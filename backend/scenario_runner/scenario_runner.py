...
class ScenarioRunner(object):
    ...
    def _load_and_wait_for_world(self, town, ego_vehicles=None):
        ...
        if self._args.reloadWorld:
            ...
            # === 修复 Carla 无法直接加载本地外部 .xodr 文件的 Bug ===
            if str(town).endswith('.xodr'):
                import os
                import io
                if os.path.exists(town):
                    print(f"[ScenarioRunner] 正在动态生成外部 OpenDRIVE 地图: {town}")
                    with io.open(town, 'r', encoding="utf-8") as od_file:
                        xodr_data = od_file.read()
                    # 调用专门解析纯文本 XODR 数据的接口
                    self.world = self.client.generate_opendrive_world(xodr_data)
                else:
                    raise FileNotFoundError(f"找不到指定的外部 OpenDRIVE 文件: {town}")
            else:
                # 原本的默认逻辑，用于加载内置地图 (如 Town01)
                self.world = self.client.load_world(town)
            # ==========================================================
        else:
